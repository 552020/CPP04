...
void test_amateria()
{
	std::cout << "Amatia is an abstract class, so we can't create an instance of it.\n";
...
void test_clone_random()
{
	...
	int choice = rand() % 2; // Randomly choose 0 (Ice) or 1 (Cure)
	...
	// Clone the created materia
	clonedMateria = materia->clone();

	// Simulate using both materias
...
void test_clone_user_input()
{
	...
	while (userInput != "ice" && userInput != "cure")
	{
		...
		// Based on user input, create an Ice or Cure materia
		if (userInput == "ice")
			materia = new Ice();
